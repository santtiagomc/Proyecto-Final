...
async function getOrders({ sort, searchValue }) {
  try {
    ...
    sort === "status-A-Z" &&
      allCarts.sort((a, b) => a.status.localeCompare(b.status));
    sort === "status-Z-A" &&
      allCarts.sort((b, a) => a.status.localeCompare(b.status));
    sort === "name-A-Z" &&
      allCarts.sort((a, b) => a.User.fullName.localeCompare(b.User.fullName));
    sort === "name-Z-A" &&
      allCarts.sort((b, a) => a.User.fullName.localeCompare(b.User.fullName));
    sort === "price-min-max" &&
      allCarts.sort(
        (a, b) =>
          a.Books.reduce((acc, el) => {
            acc = acc + el.Books_Carts.quantity * el.price;
            return acc;
          }, 0) -
          b.Books.reduce((acc, el) => {
            acc = acc + el.Books_Carts.quantity * el.price;
            return acc;
          }, 0)
      );
